...
if (isset($_POST['register'])) {
    ...
    $health = filter_var($_POST['health_info'], FILTER_SANITIZE_STRING);
    ...
    if ($password !== $cpassword && $email !== $c_email) {
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            // Hash the password
                ...
                $query = "INSERT INTO `patient` (FirstName,LastName,MiddleName,DOB,Gender,Address,Password,Email,PhoneNumber,Health,verify) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
                $stmt = mysqli_prepare($conn, $query);
                mysqli_stmt_bind_param($stmt, "sssssssssss", $firstName,$lastName,$middleName,$DOB,$gender,$address,$hashed_password,$email,$number,$health,$verify);
            ...
            if (mysqli_stmt_execute($stmt)){
                ...
                try {
                    ...
                    $mail->Body    = 'Click this link to verify your email address and login <b><a href="http://localhost/TeamRocketMMP-main2/log-in.php?verification='.$verify.'">http://localhost/Team/?verification='.$verify.'</a></b>';
